export class AdndDice {
  ...
  static async sendRoll({
    ...
    const template = "systems/adnd/templates/chat/roll-result.html";
    ...
    templateData.result = AdndDice.digestResult(data, roll);

    return new Promise((resolve) => {
      roll.render().then((r) => {
        templateData.rollADND = r;
  ...
  static digestAttackResult(data, roll) {
    ...
    if (game.settings.get("adnd", "ascendingAC")) {
      if (roll.total < targetAac) {
        result.details = game.i18n.format(
          "ADND.messages.AttackAscendingFailure",
          {
            bonus: result.target,
          }
        );
      ...
      result.details = game.i18n.format("ADND.messages.AttackAscendingSuccess", {
        result: roll.total,
      });
      result.isSuccess = true;
    } else {
      ...
      if (result.target - roll.total > targetAc) {
        result.details = game.i18n.format("ADND.messages.AttackFailure", {
          bonus: result.target,
        });
        return result;
      }
      ...
      result.details = game.i18n.format("ADND.messages.AttackSuccess", {
        result: value,
        bonus: result.target,
      });
    }
  ...
  static async sendAttackRoll({
    ...
    const template = "systems/adnd/templates/chat/roll-attack.html";
    ...
    templateData.result = AdndDice.digestAttackResult(data, roll);
  ...
  static async RollSave({
    ...
    let rolled = false;
    const template = "systems/adnd/templates/chat/roll-dialog.html";
    let dialogData = {
      formula: parts.join(" "),
      data: data,
    ...
    if (skipDialog) { return AdndDice.sendRoll(rollData); }

    let buttons = {
      ok: {
        label: game.i18n.localize("ADND.Roll"),
        icon: '<i class="fas fa-dice-d20"></i>',
        callback: (html) => {
          rolled = true;
          rollData.form = html[0].querySelector("form");
          roll = AdndDice.sendRoll(rollData);
        },
      },
      magic: {
        label: game.i18n.localize("ADND.saves.magic.short"),
        icon: '<i class="fas fa-magic"></i>',
        callback: (html) => {
          rolled = true;
          rollData.form = html[0].querySelector("form");
          rollData.data.roll.target = parseInt(rollData.data.roll.target) + parseInt(rollData.data.roll.magic);
          rollData.title += ` ${game.i18n.localize("ADND.saves.magic.short")} (${rollData.data.roll.magic})`;
          roll = AdndDice.sendRoll(rollData);
        },
      },
      cancel: {
        icon: '<i class="fas fa-times"></i>',
        label: game.i18n.localize("ADND.Cancel"),
        callback: (html) => { },
      },
    };
  ...
  static async Roll({
    ...
    if (skipDialog) {
      return ["melee", "missile", "attack"].includes(data.roll.type)
        ? AdndDice.sendAttackRoll(rollData)
        : AdndDice.sendRoll(rollData);
    }
