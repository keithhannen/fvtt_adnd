export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("adnd", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("adnd", "treasure");
    table.object.setFlag("adnd", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('ADND.table.treasure.roll')}</button>`;
...
function drawTreasure(table, data) {
  ...
  data.treasure = {};
  if (table.getFlag('adnd', 'treasure')) {
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/adnd/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    // sound: "/systems/adnd/assets/coins.mp3"
  }
