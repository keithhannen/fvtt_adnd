 ...
 */
export class AdndActorSheetMonster extends AdndActorSheet {
  constructor(...args) {
    super(...args);
  }
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["adnd", "sheet", "monster", "actor"],
      template: "systems/adnd/templates/actors/monster-sheet.html",
      width: 450,
      height: 560,
      resizable: true,
      tabs: [
   ...
   */
  async generateSave() {
    let choices = CONFIG.ADND.monster_saves;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/adnd/templates/actors/dialogs/monster-saves.html",
        templateData
      );
    //Create Dialog window
    new Dialog({
      title: game.i18n.localize("ADND.dialog.generateSaves"),
      content: dlg,
      buttons: {
        ok: {
  ...
  getData() {
    ...
    // Settings
    data.config.morale = game.settings.get("adnd", "morale");
