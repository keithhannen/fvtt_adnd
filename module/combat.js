export class AdndCombat {
  static rollInitiative(combat, data) {
    // Check groups
    data.combatants = [];
    let groups = {};
    combat.data.combatants.forEach((cbt) => {
      groups[cbt.flags.adnd.group] = { present: true };
    ...
    Object.keys(groups).forEach((group) => {
      ...
      roll.toMessage({
        flavor: game.i18n.format('ADND.roll.initiative', { group: CONFIG["ADND"].colors[group] }),
      });
    ...
    for (let i = 0; i < data.combatants.length; ++i) {
      ...
      if (data.combatants[i].actor.data.data.isSlow) {
        ...
        data.combatants[i].initiative =
          groups[data.combatants[i].flags.adnd.group].initiative;
      }
    }
    combat.setupTurns();
  }
  ...
  static async resetInitiative(combat, data) {
    let reroll = game.settings.get("adnd", "rerollInitiative");
  ...
  static async individualInitiative(combat, data) {
    ...
    combat.data.combatants.forEach((c, i) => {
      ...
      let messageData = mergeObject({
        ...
        flavor: game.i18n.format('ADND.roll.individualInit', { name: c.token.name })
      }, {});
  ...
  static format(object, html, user) {
    ...
    html.find(".combatant").each((_, ct) => {
      ...
      const moveActive = cmbtant.flags.adnd && cmbtant.flags.adnd.moveInCombat ? "active" : "";
      controls.eq(1).after(
        `<a class='combatant-control move-combat ${moveActive}'><i class='fas fa-walking'></i></a>`
      );
      const spellActive = cmbtant.flags.adnd && cmbtant.flags.adnd.prepareSpell ? "active" : "";
      controls.eq(1).after(
        `<a class='combatant-control prepare-spell ${spellActive}'><i class='fas fa-magic'></i></a>`
      );
    });
    AdndCombat.announceListener(html);

    let init = game.settings.get("adnd", "initiative") === "group";
    if (!init) {
      return;
    }
    ...
    html.find(".combatant").each((_, ct) => {
      ...
      // Get group color
      const cmbtant = object.combat.getCombatant(ct.dataset.combatantId);
      let color = cmbtant.flags.adnd.group;

      // Append colored flag
      let controls = $(ct).find(".combatant-controls");
      controls.prepend(
        `<a class='combatant-control flag' style='color:${color}' title="${CONFIG.ADND.colors[color]}"><i class='fas fa-flag'></i></a>`
      );
    });
    AdndCombat.addListeners(html);
  }
    ...
    if (data.initiative && init == "group") {
      ...
      combat.combatants.forEach((ct) => {
        if (
          ...
          ct._id != data._id &&
          ct.flags.adnd.group == combatant.flags.adnd.group
        ) {
  ...
  static announceListener(html) {
    html.find(".combatant-control.prepare-spell").click((ev) => {
      ...
      game.combat.updateCombatant({
        _id: id,
        flags: { adnd: { prepareSpell: !isActive } },
      });
    });
    html.find(".combatant-control.move-combat").click((ev) => {
      ...
      game.combat.updateCombatant({
        _id: id,
        flags: { adnd: { moveInCombat: !isActive } },
      });
    })
  }
  ...
  static addListeners(html) {
    ...
    html.find(".combatant-control.flag").click((ev) => {
      ...
      let colors = Object.keys(CONFIG.ADND.colors);
    ...
    html.find('.combat-control[data-control="reroll"]').click((ev) => {
      ...
      let data = {};
      AdndCombat.rollInitiative(game.combat, data);
  ...
  static addContextEntry(html, options) {
    options.unshift({
      ...
      callback: AdndCombat.activateCombatant
    });
  }
  ...
  static async preUpdateCombat(combat, data, diff, id) {
    ...
    if (data.round !== 1) {
      if (reroll === "reset") {
        AdndCombat.resetInitiative(combat, data, diff, id);
        return;
      } else if (reroll === "keep") {
        return;
      }
    ...
    if (init === "group") {
      AdndCombat.rollInitiative(combat, data, diff, id);
    } else if (init === "individual") {
      AdndCombat.individualInitiative(combat, data, diff, id);
    }
  }
}
