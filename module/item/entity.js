 ...
 */
export class AdndItem extends Item {
  /* -------------------------------------------- */
  /*	Data Preparation														*/
  /* -------------------------------------------- */
  /**
  ...
  prepareData() {
    ...
    switch (this.data.type) {
      case "spell":
        img = "/systems/adnd/assets/default/spell.png";
        break;
      case "ability":
        img = "/systems/adnd/assets/default/ability.png";
        break;
      case "armor":
        img = "/systems/adnd/assets/default/armor.png";
        break;
      case "weapon":
        img = "/systems/adnd/assets/default/weapon.png";
        break;
      case "item":
        img = "/systems/adnd/assets/default/item.png";
        break;
    }
  ...
  async rollFormula(options = {}) {
    ...
    return AdndDice.Roll({
      ...
      flavor: game.i18n.format("ADND.roll.formula", { label: label }),
      title: game.i18n.format("ADND.roll.formula", { label: label }),
    });
  }
  ...
  getTags() {
    ...
    switch (this.data.type) {
      case "weapon":
        ...
        wTags += formatTag(CONFIG.ADND.saves_long[data.save], "fa-skull");
        if (data.missile) {
          wTags += formatTag(
        ...
        return wTags;
      case "armor":
        return `${formatTag(CONFIG.ADND.armor[data.type], "fa-tshirt")}`;
      case "item":
        return "";
      case "spell":
        ...
        if (data.save) {
          sTags += formatTag(CONFIG.ADND.saves_long[data.save], "fa-skull");
        }
        return sTags;
      case "ability":
        let roll = "";
        roll += data.roll ? data.roll : "";
        roll += data.rollTarget ? CONFIG.ADND.roll_type[data.rollType] : "";
  ...
  pushTag(values) {
    ...
    if (update) {
      values.forEach((val) => {
        ...
        switch (val) {
          case CONFIG.ADND.tags.melee:
            newData.melee = true;
            break;
          case CONFIG.ADND.tags.slow:
            newData.slow = true;
            break;
          case CONFIG.ADND.tags.missile:
  ...
  async show() {
    ...
    const template = `systems/adnd/templates/chat/item-card.html`;
