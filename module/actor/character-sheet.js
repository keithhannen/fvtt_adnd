import { AdndActor } from "./entity.js";
import { AdndActorSheet } from "./actor-sheet.js";
import { AdndCharacterModifiers } from "../dialog/character-modifiers.js";
import { AdndCharacterCreator } from "../dialog/character-creation.js";

/**
 * Extend the basic ActorSheet with some very simple modifications
 */
export class AdndActorSheetCharacter extends AdndActorSheet {
  constructor(...args) {
    super(...args);
  }
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["adnd", "sheet", "actor", "character"],
      template: "systems/adnd/templates/actors/character-sheet.html",
      width: 450,
      height: 530,
      resizable: true,
      tabs: [
  ...
  generateScores() {
    new AdndCharacterCreator(this.actor, {
  ...
  getData() {
    ...
    data.config.ascendingAC = game.settings.get("adnd", "ascendingAC");
    data.config.initiative = game.settings.get("adnd", "initiative") != "group";
    data.config.encumbrance = game.settings.get("adnd", "encumbranceOption");
  ...
  async _chooseLang() {
    let choices = CONFIG.ADND.languages;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/adnd/templates/actors/dialogs/lang-create.html",
        templateData
      );
    ...
    return new Promise((resolve) => {
      new Dialog({
        ...
        buttons: {
          ok: {
            label: game.i18n.localize("ADND.Ok"),
  ...
  _pushLang(table) {
    ...
    this._chooseLang().then((dialogInput) => {
      const name = CONFIG.ADND.languages[dialogInput.choice];
      if (update.value) {
  ...
  _onShowModifiers(event) {
    event.preventDefault();
    new AdndCharacterModifiers(this.actor, {
      top: this.position.top + 40,
      left: this.position.left + (this.position.width - 400) / 2,
    }).render(true);
  }
