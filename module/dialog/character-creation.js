...
export class AdndCharacterCreator extends FormApplication {
  static get defaultOptions() {
    const options = super.defaultOptions;
    options.classes = ["adnd", "dialog", "creator"],
      options.id = 'character-creator';
    options.template =
      'systems/adnd/templates/actors/dialogs/character-creation.html';
    options.width = 235;
    return options;
  }
   ...
   */
  get title() {
    return `${this.object.name}: ${game.i18n.localize('ADND.dialog.generator')}`;
  }
  ...
  rollScore(score, options = {}) {
    ...
    const label = score != "gold" ? game.i18n.localize(`ADND.scores.${score}.long`) : "Gold";
    const rollParts = ["3d6"];
    const data = {
      roll: {
        type: "result"
    ...
    return AdndDice.Roll({
      ...
      flavor: game.i18n.format('ADND.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
      title: game.i18n.format('ADND.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
    });
  }
  ...
  async close() {
    ...
    const templateData = {
      config: CONFIG.ADND,
      scores: scores,
      title: game.i18n.localize("ADND.dialog.generator"),
      stats: this.object.data.stats,
      gold: gold
    }
    const content = await renderTemplate("/systems/adnd/templates/chat/roll-creation.html", templateData)
    ChatMessage.create({
      content: content,
      speaker,
    });
