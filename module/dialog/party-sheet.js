import { AdndPartyXP } from "./party-xp.js";

export class AdndPartySheet extends FormApplication {

  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["adnd", "dialog", "party-sheet"],
      template: "systems/adnd/templates/apps/party-sheet.html",
      width: 280,
      height: 400,
      resizable: true,
    });
   ...
   */
  get title() {
    return game.i18n.localize("ADND.dialog.partysheet");
  }
  ...
  getData() {
    const settings = {
      ascending: game.settings.get('adnd', 'ascendingAC')
    };
    let data = {
      data: this.object,
      config: CONFIG.ADND,
  ...
  async _dealXP(ev) {
    new AdndPartyXP(this.object, {}).render(true);
  }

  async _selectActors(ev) {
    const template = "/systems/adnd/templates/apps/party-select.html";
    const templateData = {
